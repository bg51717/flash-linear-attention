...
from fla.ops.simple_gla import chunk_simple_gla, fused_chunk_simple_gla, fused_recurrent_simple_gla
...
def css_feature_map(
    ...
    """Compact Symmetric Squared kernel feature map.

    Projects x to R^{d_r}, optionally L2-normalizes, then computes
    vech_sqrt2(g g^T).  Guarantees phi(q)^T phi(k) = (g_q^T g_k)^2 >= 0.

    Args:
        x: [*, d_head]
        proj_weight: [d_r, d_head]
        triu_i, triu_j: [M] upper-triangle index vectors
        triu_scale: [M]  1.0 on diagonal, sqrt(2) off-diagonal
        normalize: whether to L2-normalize the projected vector
        norm_eps: epsilon for normalization
    Returns:
        [*, M]  where M = d_r(d_r+1)/2
    """
    g = F.linear(x, proj_weight)
    if normalize:
        g = F.normalize(g, p=2, dim=-1, eps=norm_eps)
    return g[..., triu_i] * g[..., triu_j] * triu_scale


def css_recurrence_naive(
    phi_q: torch.Tensor,
    phi_k: torch.Tensor,
    v: torch.Tensor,
    ...
    Args:
        ...
        v:     [B, H, T, DV]
        log_gamma: [H]
        initial_state: (S[B,H,M,DV], z[B,H,M]) or None
    Returns:
        o: [B, H, T, DV]
        final_state: (S, z) or None
    """
    orig_dtype = phi_q.dtype
    b, h, t, m = phi_q.shape
    d_v = v.shape[-1]
    phi_q, phi_k, v = phi_q.float(), phi_k.float(), v.float()
    gamma = torch.exp(log_gamma.float())

    o = torch.zeros_like(v)
    S = torch.zeros(b, h, m, d_v, device=phi_q.device, dtype=torch.float32)
    z = torch.zeros(b, h, m, device=phi_q.device, dtype=torch.float32)
    if initial_state is not None:
        S = S + initial_state[0].float()
        z = z + initial_state[1].float()
    ...
    for i in range(t):
        ...
        S = gamma[None, :, None, None] * S + pk.unsqueeze(-1) * vi.unsqueeze(-2)
        z = gamma[None, :, None] * z + pk

        num = torch.einsum('bhm,bhmd->bhd', pq, S)
        den = torch.einsum('bhm,bhm->bh', pq, z).unsqueeze(-1) + eps
        o[:, :, i] = num / den

    final = (S, z) if output_final_state else None
...
def css_linear_attention(
    ...
    dtype = v.dtype
    phi_q = phi_q.to(dtype)
    phi_k = phi_k.to(dtype)
    g_gamma = log_gamma.float()

    if mode not in _BACKENDS:
        raise ValueError(f"Unsupported mode `{mode}`. Choose from {list(_BACKENDS.keys())}.")
    backend = _BACKENDS[mode]

    num_h0, den_h0 = None, None
    if initial_state is not None:
        S0, z0 = initial_state
        num_h0 = S0
        den_h0 = z0.unsqueeze(-1)

    num, num_ht = backend(
        q=phi_q, k=phi_k, v=v, g_gamma=g_gamma, scale=1.0,
        initial_state=num_h0,
        output_final_state=output_final_state,
        cu_seqlens=cu_seqlens,
    )

    ones = v.new_ones(B, T, H, 1)
    den_out, den_ht = backend(
        q=phi_q, k=phi_k, v=ones, g_gamma=g_gamma, scale=1.0,
        initial_state=den_h0,
        output_final_state=output_final_state,
        cu_seqlens=cu_seqlens,
    )
    ...
    final_state = None
    if output_final_state and num_ht is not None:
        final_state = (num_ht, den_ht.squeeze(-1))

    return o, final_state


class CSSLinearAttention(nn.Module):
    ...
    def __init__(
        ...
        proj_normalize: bool = True,
        proj_norm_eps: float = 1e-6,
        ...
        **kwargs,
        ...
        self.proj_normalize = proj_normalize
        self.proj_norm_eps = proj_norm_eps
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        _fm_args = (self.proj_weight, self.triu_idx_i, self.triu_idx_j, self.triu_scale)
        _fm_kwargs = dict(normalize=self.proj_normalize, norm_eps=self.proj_norm_eps)
        if self.training:
            phi_q = grad_checkpoint(css_feature_map, q, *_fm_args, **_fm_kwargs, use_reentrant=False)
            phi_k = grad_checkpoint(css_feature_map, k, *_fm_args, **_fm_kwargs, use_reentrant=False)
        else:
            phi_q = css_feature_map(q, *_fm_args, **_fm_kwargs)
            phi_k = css_feature_map(k, *_fm_args, **_fm_kwargs)

        log_gamma = F.logsigmoid(self.raw_decay)

        o, recurrent_state = css_linear_attention(
            phi_q=phi_q,
            phi_k=phi_k,
            v=v,
            log_gamma=log_gamma,
