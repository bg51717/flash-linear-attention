...
def hpk_feature_map(
    x: torch.Tensor,
    proj_weights: list[torch.Tensor],
) -> torch.Tensor:
    """Hadamard Power Kernel feature map.

    φ(x) = Π_{p=1}^{P} (elu(W_p x) + 1)

    Args:
        x: [*, d_head]
        proj_weights: list of P tensors, each [feature_dim, d_head]
    Returns:
        [*, feature_dim]
    """
    out = F.elu(F.linear(x, proj_weights[0]), alpha=1.0) + 1.0
    for w in proj_weights[1:]:
        out = out * (F.elu(F.linear(x, w), alpha=1.0) + 1.0)
    return out


def hpk_recurrence_naive(
    phi_q: torch.Tensor,
    phi_k: torch.Tensor,
    v: torch.Tensor,
...
def hpk_linear_attention(
    ...
    """HPK linear attention using FLA's fused recurrent kernel.

    Trick: augment v with a column of ones so that a single kernel call
    computes both the numerator (phi_q^T S_t) and the normalizer (phi_q^T z_t).

    Args:
        phi_q: [B, T, H, M]  (pre-computed feature map)
        phi_k: [B, T, H, M]
        v:     [B, T, H, DV]
        log_gamma: [H]  log-domain per-head decay
        initial_state: (S[N,H,M,DV], z[N,H,M]) or None
        cu_seqlens: [N+1] for variable-length
    Returns:
        o: [B, T, H, DV]
        final_state: (S[N,H,M,DV], z[N,H,M]) or None
    """
    B, T, H, M = phi_q.shape
    DV = v.shape[-1]
...
class HPKLinearAttention(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.proj_weights = nn.ParameterList([
            nn.Parameter(torch.empty(feature_dim, self.head_k_dim))
            for _ in range(power_order)
        ])
        for w in self.proj_weights:
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        proj_list = list(self.proj_weights)
        phi_q = hpk_feature_map(q, proj_list)
        phi_k = hpk_feature_map(k, proj_list)

        log_gamma = F.logsigmoid(self.raw_decay)

        o, recurrent_state = hpk_linear_attention(
            phi_q=phi_q,
            phi_k=phi_k,
            v=v,
            log_gamma=log_gamma,
