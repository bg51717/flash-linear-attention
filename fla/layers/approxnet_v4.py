...
def _prepare_initial_state(
    ...
    if initial_state is None:
        ...
        m2 = torch.zeros((n,), device=device, dtype=dtype)
        return d_state, nu, kap, o_prev, q_prev, count, mu, m2

    if len(initial_state) != 8:
        raise ValueError(
            "initial_state must be an 8-tuple: (D, nu, kappa, o_prev, q_prev, count, mu, m2).",
        )
    d_state, nu, kap, o_prev, q_prev, count, mu, m2 = initial_state
    ...
    mu = mu.reshape(n).to(device=device, dtype=dtype).contiguous()
    m2 = m2.reshape(n).to(device=device, dtype=dtype).contiguous()
    return d_state, nu, kap, o_prev, q_prev, count, mu, m2


def _torch_approxnet_v4_dense(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    ...
    d_state, nu, kap, o_prev, q_prev, count, mu, m2 = _prepare_initial_state(qf, vf, initial_state)
    ...
    for i in range(t):
        ...
        mu_new = mu + delta1.detach() / c_t
        delta2 = score.detach() - mu_new
        m2_new = m2 + delta1.detach() * delta2
    ...
    final_state = (d_state, nu, kap, o_prev, q_prev, count, mu, m2) if output_final_state else None
    stats = {
        "approxnet_v4_beta_mean": beta_mean_sum / max(t, 1),
    }
...
def _flatten_initial_state(
    ...
    return (
        ...
        mu.reshape(batch * heads).contiguous(),
        m2.reshape(batch * heads).contiguous(),
    )
...
def _reshape_final_state(
    ...
    d_state, nu, kap, o_prev, q_prev, count, mu, m2 = final_state
    return (
        ...
        mu.reshape(out_batch, n_heads).contiguous(),
        m2.reshape(out_batch, n_heads).contiguous(),
    )
...
def _run_varlen(
    ...
    if initial_state is not None:
        d0, nu0, kap0, o0, qp0, c0, mu0, m20 = initial_state
        init_flat = (
            ...
            mu0.reshape(n_seq, n_heads).contiguous(),
            m20.reshape(n_seq, n_heads).contiguous(),
        )
    else:
        init_flat = None
    ...
    for i in range(n_seq):
        ...
        out_seg, st_seg, stats_seg = _torch_approxnet_v4_dense(
            q=qf[:, bos:eos, :],
            k=kf[:, bos:eos, :],
            v=vf[:, bos:eos, :],
        ...
        out[:, bos:eos, :] = out_seg
        beta_means.append(stats_seg["approxnet_v4_beta_mean"])
        if output_final_state:
            for idx in range(8):
    ...
    stats = {
        "approxnet_v4_beta_mean": float(sum(beta_means) / max(len(beta_means), 1)),
    }
    return out, final_state, stats


def approxnet_v4_linear_attention(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    gate_alpha: torch.Tensor,
    ...
    ga = gate_alpha.repeat(batch)
    gb = gate_bias.repeat(batch)

    init_flat = _flatten_initial_state(initial_state, batch, n_heads, v_dim, k_dim)
    if cu_seqlens is None:
        out_flat, final_flat, stats = _torch_approxnet_v4_dense(
            q=qf,
            k=kf,
            v=vf,
            gate_alpha=ga,
...
class ApproxNetV4LinearAttention(nn.Module):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        self.gate_alpha = nn.Parameter(torch.full((num_heads,), gate_alpha_init))
        self.gate_bias = nn.Parameter(torch.full((num_heads,), gate_bias_init))
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        use_triton_path = bool(
            self.use_triton
            and _APPROXNET_V4_TRITON_AVAILABLE
            and q.is_cuda
            and not (stateful and grad_enabled)
        )
        if use_triton_path:
            o, recurrent_state, stats = approxnet_v4_linear_attention_triton(
                q=q,
                k=k,
                v=v,
                ...
                cu_seqlens=cu_seqlens,
                z_score_eps=self.z_score_eps,
                recompute_chunk_size=self.recompute_chunk_size,
            )
        else:
            o, recurrent_state, stats = approxnet_v4_linear_attention(
                q=q,
                k=k,
                v=v,
                gate_alpha=self.gate_alpha,
