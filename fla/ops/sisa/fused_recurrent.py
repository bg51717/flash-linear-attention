"""Triton fused recurrent kernels for SiSA (Softmax-in-State Attention)."""
...
def _fused_recurrent_sisa_fwd_kernel(
    q_r, k_r, v, alpha, beta, sm_beta,
    o, q_n_out,
    h0_S, h0_G,
    ht_S, ht_G,
    cu_seqlens,
    ...
    b_sm_beta = tl.load(sm_beta + i_h).to(tl.float32)
    ...
    for _ in range(0, T):
        ...
        # SiSA readout: softmax gating
        b_Gq = tl.sum(b_G * b_q[None, :], axis=1)       # [DR]
        b_s = b_sm_beta * b_Gq                             # [DR]
        b_s_max = tl.max(b_s, axis=0)
        b_exp = tl.exp(b_s - b_s_max)
        b_w = b_exp / tl.sum(b_exp)                        # [DR]

        b_qt = b_q * b_w                                   # [DR]
        b_qt_norm = tl.sqrt(tl.sum(b_qt * b_qt) + 1e-12)
        b_qn = b_qt / b_qt_norm                            # [DR]
...
# Phase 1 (reverse): propagate dS backward, compute dk, dv, dbeta_gate
# Phase 2 (forward): recompute state, compute dq and d_sm_beta via softmax backward
# dalpha always zeros (decay gate frozen/detached)
# dG always zeros (Gram detached during softmax gating)
# ---------------------------------------------------------------------------
...
@triton.jit(do_not_specialize=['T'])
def _fused_recurrent_sisa_bwd_kernel(
    q_r, k_r, v,
    alpha, beta, sm_beta,
    q_n_saved,
    do_grad,
    ...
    dh0_S,
    dht_S,
    dq, dk, dv_out, dbeta_out, dsm_beta_out,
    cu_seqlens,
    T,
    B: tl.constexpr,
    H: tl.constexpr,
    ...
    # Recompute S, G. Compute dq and d_sm_beta via softmax backward.
    b_S = tl.zeros([DR, BDV], dtype=tl.float32)
    b_G = tl.zeros([DR, DR], dtype=tl.float32)
    b_dsm_beta_accum = 0.0
    ...
    for _ in range(T):
        ...
        # dq_n partial from this DV block
        b_dqn_partial = tl.sum(b_S * b_do_val[None, :], axis=1)

        # Recompute softmax gating for backward
        b_Gq = tl.sum(b_G * b_q[None, :], axis=1)
        b_s = b_sm_beta * b_Gq
        b_s_max = tl.max(b_s, axis=0)
        b_exp = tl.exp(b_s - b_s_max)
        b_w = b_exp / tl.sum(b_exp)
        b_qt = b_q * b_w
        b_qt_norm = tl.sqrt(tl.sum(b_qt * b_qt) + 1e-12)

        # L2norm backward: qn = qt / ||qt||
        b_proj = tl.sum(b_qn * b_dqn_partial)
        b_dqt = (b_dqn_partial - b_qn * b_proj) / b_qt_norm

        # Element-wise backward: qt = q * w
        b_dq_from_qt = b_dqt * b_w
        b_dw = b_dqt * b_q

        # Softmax backward: w = softmax(sm_beta * Gq)
        b_wdw = tl.sum(b_w * b_dw)
        b_ds = b_w * (b_dw - b_wdw)

        # s = sm_beta * Gq backward
        b_dGq = b_sm_beta * b_ds
        b_dsm_beta_accum = b_dsm_beta_accum + tl.sum(b_Gq * b_ds)

        # Gq = G @ q backward (G detached, only through q)
        b_dq_from_Gq = tl.sum(b_G * b_dGq[:, None], axis=0)

        b_dq_partial = b_dq_from_qt + b_dq_from_Gq
    ...
    tl.store(dsm_beta_out + i_dv * B * H + i_nh, b_dsm_beta_accum)


def fused_recurrent_sisa_fwd(
    q_r: torch.Tensor,
    k_r: torch.Tensor,
    v: torch.Tensor,
    alpha: torch.Tensor,
    ...
    grid = (NV, N * H)
    _fused_recurrent_sisa_fwd_kernel[grid](
        q_r, k_r, v, alpha, beta, sm_beta,
        o, q_n_saved,
        h0_S, h0_G,
        ht_S, ht_G,
...
def fused_recurrent_sisa_bwd(
    ...
    dsm_beta_out = q_r.new_empty(NV, B * H)
    ...
    grid = (NV, N * H)
    _fused_recurrent_sisa_bwd_kernel[grid](
        q_r, k_r, v,
        alpha, beta, sm_beta,
        q_n_saved,
        do_grad,
    ...
    dsm_beta = dsm_beta_out.sum(0).reshape(B, H).sum(0)

    return dq, dk, dv_out, dalpha, dbeta, dsm_beta, dh0_S


class FusedRecurrentSiSAFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        o, q_n, ht_S, ht_G = fused_recurrent_sisa_fwd(
            q_r, k_r, v, alpha, beta, sm_beta,
            initial_state,
        ...
        ctx.save_for_backward(q_r, k_r, v, alpha, beta, sm_beta, q_n)
        if initial_state is not None:
    ...
    def backward(ctx, do, d_final_state):
        q_r, k_r, v, alpha, beta, sm_beta, q_n = ctx.saved_tensors
        ...
        dq, dk, dv, dalpha, dbeta, dsm_beta, dh0_S = fused_recurrent_sisa_bwd(
            q_r, k_r, v, alpha, beta, sm_beta, q_n,
            do,
            initial_state, dht,
        ...
        return (dq.to(q_r), dk.to(k_r), dv.to(v),
                dalpha.to(alpha), dbeta.to(beta), dsm_beta.to(sm_beta),
                dh0, None, None)


@torch.compiler.disable
def fused_recurrent_sisa(
    q_r: torch.Tensor,
    k_r: torch.Tensor,
    v: torch.Tensor,
    alpha: torch.Tensor,
    ...
    o, final_state = FusedRecurrentSiSAFunction.apply(
        q_r, k_r, v, alpha, beta, sm_beta,
