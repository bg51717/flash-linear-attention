...
if _TRITON_AVAILABLE:
    ...
    @triton.jit(do_not_specialize=["T"])
    def _approxnet_v3_fwd_state_kernel(
        q_ptr,
        k_ptr,
        v_ptr,
        o_ptr,
    ...
    def _approxnet_v3_chunk_state_kernel(
        q_ptr,
        k_ptr,
        v_ptr,
        d_in_ptr,
    ...
    @triton.jit(do_not_specialize=["T", "t_start", "L"])
    def _approxnet_v3_chunk_states_kernel(
        q_ptr,
        k_ptr,
        v_ptr,
        d_in_ptr,
    ...
    @triton.jit(do_not_specialize=["T", "t_start", "L"])
    def _approxnet_v3_chunk_bwd_kernel(
        q_ptr,
        k_ptr,
        v_ptr,
        do_ptr,
        ...
        for _ in range(0, L):
            ...
            # v3: scale by inv_count
            b_inv_count = tl.where(b_c_prev > 0.0, 1.0 / b_c_prev, 0.0)

            b_dq_step = b_q - b_q_prev
            b_h = b_o_prev + b_inv_count * tl.sum(b_d_t * b_dq_step[None, :], axis=1)
            ...
            # h path: o_hist = o_prev + inv_count * D_t @ dq_step
            # v3: dh scaled by inv_count
    ...
    @triton.jit(do_not_specialize=["T", "t_start", "L"])
    def _approxnet_v3_chunk_fused_bwd_kernel(
        q_ptr,
        k_ptr,
        v_ptr,
        do_ptr,
        ...
        b_r = tl.load(cp_r_ptr + i * K + offs_k, mask=mask_k, other=0).to(tl.float32)
        ...
        for _ in range(0, L):
            ...
            b_r_pv = tl.load(p_r_prev, mask=mask_k, other=0).to(tl.float32)
            ...
            if SCORE_CLIP > 0:
                b_score = tl.minimum(tl.maximum(b_score_raw, -SCORE_CLIP), SCORE_CLIP)
                b_clip_mask = tl.where((b_score_raw >= -SCORE_CLIP) & (b_score_raw <= SCORE_CLIP), 1.0, 0.0)
            b_den = b_c_pv + tl.sum(b_r_pv * b_q, axis=0) + BETA_DENOM_EPS
            b_beta = tl.exp(b_score) / b_den
            ...
            b_g_score = b_d_beta * b_beta * b_clip_mask
            b_g_den = -b_d_beta * b_beta / b_den
            b_dk_out += b_g_score * b_q
            b_dq_out += b_g_score * b_k + b_g_den * b_r_pv
            b_dr_pv = b_dr + b_g_den * b_q
            ...
            b_dnu_pv += b_dnu * (1.0 - b_alpha)
            b_dv_out += b_dnu * b_alpha
            b_dkap_pv += b_dkap * (1.0 - b_alpha)
            b_dk_out += b_dkap * b_alpha
...
def _fwd_triton_with_state(
    ...
    _approxnet_v3_fwd_state_kernel[(n,)](
        q_ptr=q,
        k_ptr=k,
        v_ptr=v,
        o_ptr=out,
...
def _bwd_triton_dense(
    ...
    for c in range(n_chunks):
        s = c * chunk
        l = min(chunk, t - s)
        _approxnet_v3_chunk_state_kernel[(n,)](
            q_ptr=qf, k_ptr=kf, v_ptr=vf,
            d_in_ptr=cp_d[c], nu_in_ptr=cp_nu[c], kap_in_ptr=cp_kap[c],
            r_in_ptr=cp_r[c], o_in_ptr=cp_o[c], qp_in_ptr=cp_qp[c], c_in_ptr=cp_c[c],
            d_out_ptr=cp_d[c + 1], nu_out_ptr=cp_nu[c + 1], kap_out_ptr=cp_kap[c + 1],
            r_out_ptr=cp_r[c + 1], o_out_ptr=cp_o[c + 1], qp_out_ptr=cp_qp[c + 1], c_out_ptr=cp_c[c + 1],
            t_start=s, L=l, T=t,
            BETA_DENOM_EPS=float(beta_denom_eps), SCORE_CLIP=float(score_clip),
    ...
    for c in range(n_chunks - 1, -1, -1):
        s = c * chunk
        l = min(chunk, t - s)
        _approxnet_v3_chunk_fused_bwd_kernel[(n,)](
            q_ptr=qf, k_ptr=kf, v_ptr=vf, do_ptr=dof,
            cp_d_ptr=cp_d[c], cp_nu_ptr=cp_nu[c], cp_kap_ptr=cp_kap[c],
            cp_r_ptr=cp_r[c], cp_o_ptr=cp_o[c], cp_qp_ptr=cp_qp[c], cp_c_ptr=cp_c[c],
            dD_next_ptr=dD_next, dnu_next_ptr=dnu_next, dkap_next_ptr=dkap_next,
            dr_next_ptr=dr_next, do_next_ptr=do_next, dqp_next_ptr=dqp_next,
            dq_ptr=dqf, dk_ptr=dkf, dv_ptr=dvf,
            dD_prev_ptr=dD_prev, dnu_prev_ptr=dnu_prev, dkap_prev_ptr=dkap_prev,
            dr_prev_ptr=dr_prev, do_prev_ptr=do_prev, dqp_prev_ptr=dqp_prev,
            local_nu_ptr=local_nu, local_kap_ptr=local_kap,
            local_r_ptr=local_r, local_o_ptr=local_o,
            local_qp_ptr=local_qp, local_c_ptr=local_c,
            t_start=s, L=l, T=t,
            BETA_DENOM_EPS=float(beta_denom_eps), SCORE_CLIP=float(score_clip),
...
class _ApproxNetV3DenseFunction(torch.autograd.Function):
...
def approxnet_v3_linear_attention_triton(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    ...
    if not _TRITON_AVAILABLE:
        raise RuntimeError("ApproxNet-v3 Triton kernels are unavailable in current environment.")
    if not (q.is_cuda and k.is_cuda and v.is_cuda):
        raise RuntimeError("ApproxNet-v3 Triton kernels require CUDA tensors.")
    ...
    if torch.is_grad_enabled() and needs_backward and (initial_state is not None or output_final_state):
        raise RuntimeError("ApproxNet-v3 Triton state path currently supports inference forward only (no backward).")
    ...
    if cu_seqlens is None:
        if initial_state is None and not output_final_state and torch.is_grad_enabled():
            out_flat = _ApproxNetV3DenseFunction.apply(
                qf,
                kf,
                vf,
        ...
        for i in range(len(cu) - 1):
            ...
            if eos > bos:
                ...
                if use_autograd_dense:
                    parts.append(
                        _ApproxNetV3DenseFunction.apply(
                            qf[:, bos:eos, :],
                            kf[:, bos:eos, :],
                            vf[:, bos:eos, :],
                            float(beta_denom_eps),
