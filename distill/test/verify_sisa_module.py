"""Quick sanity check for SiSA module — forward pass, gradient flow, naive vs compiled match."""
import sys
import os
...
from distill.models.linear_attention_sisa import (
    sisa_recurrence_naive,
    _fused_recurrent_sisa,
    SiSALinearAttention,
)
...
def test_naive_forward():
    ...
    log_beta = torch.zeros(H)  # beta = 1.0

    o, state = sisa_recurrence_naive(
        q_r, k_r, v, decay_alpha, decay_bias, write_alpha, write_bias, log_beta
    )
...
def test_naive_vs_compiled():
    ...
    write_alpha = torch.ones(H)
    write_bias = torch.zeros(H)
    log_beta = torch.zeros(H)

    # Naive path
    o_naive, _ = sisa_recurrence_naive(
        q_r, k_r, v, decay_alpha, decay_bias, write_alpha, write_bias, log_beta,
    ...
    sm_beta = torch.exp(log_beta).repeat(B)

    o_compiled, _, _ = _fused_recurrent_sisa(
        q_flat, k_flat, v_flat, da, db, wa, wb, sm_beta,
    )
...
def test_gradient_flow():
    ...
    assert "log_beta" in params_with_grad, "log_beta has no gradient!"
...
if __name__ == '__main__':
    print("SiSA Module Verification")
    print("=" * 50)
