...
try:
    from distill.models.linear_attention_approxnet_v2 import (
        ApproxNetV2LinearAttention,
        approxnet_v2_linear_attention,
    )
    from distill.models.linear_attention_approxnet_v2_triton import (
        _TRITON_AVAILABLE as _APPROXNET_V2_TRITON_AVAILABLE,
        approxnet_v2_linear_attention_triton,
    )
except Exception as exc:  # pragma: no cover
    _IMPORT_ERROR = exc
    ApproxNetV2LinearAttention = None
    approxnet_v2_linear_attention = None
...
def _run_dense_chunk_equivalence_case() -> None:
    ...
    out_full, st_full, _ = approxnet_v2_linear_attention(
        q=q,
        k=k,
        v=v,
    ...
    out_a, st_a, _ = approxnet_v2_linear_attention(
        q=q[:, :6],
        k=k[:, :6],
        v=v[:, :6],
        output_final_state=True,
    ...
    out_b, st_b, _ = approxnet_v2_linear_attention(
        q=q[:, 6:],
        k=k[:, 6:],
        v=v[:, 6:],
        initial_state=st_a,
...
def _run_varlen_equivalence_case() -> None:
    ...
    out_packed, st_packed, _ = approxnet_v2_linear_attention(
        q=q,
        k=k,
        v=v,
        cu_seqlens=cu,
    ...
    for s, e in zip(cu[:-1].tolist(), cu[1:].tolist()):
        o_i, st_i, _ = approxnet_v2_linear_attention(
            q=q[:, s:e],
            k=k[:, s:e],
            v=v[:, s:e],
            output_final_state=True,
...
def _run_triton_vs_torch_case() -> None:
    if not torch.cuda.is_available() or not _APPROXNET_V2_TRITON_AVAILABLE:
        return
    torch.manual_seed(20260411 + 2)
    ...
    out_ref, _, _ = approxnet_v2_linear_attention(
        q=q_ref,
        k=k_ref,
        v=v_ref,
        beta_denom_eps=1e-1,
    ...
    out_tri, _, _ = approxnet_v2_linear_attention_triton(
        q=q_tri,
        k=k_tri,
        v=v_tri,
        beta_denom_eps=1e-1,
...
def _run_triton_stateful_dense_vs_torch_case() -> None:
    ...
    out_ref, st_ref, _ = approxnet_v2_linear_attention(
        q=q,
        k=k,
        v=v,
    ...
    out_tri, st_tri, _ = approxnet_v2_linear_attention_triton(
        q=q,
        k=k,
        v=v,
        initial_state=tuple(x.clone() for x in init_state),
...
def _run_module_smoke_case() -> None:
    if not torch.cuda.is_available():
        return
    torch.manual_seed(20260411 + 3)
    device = torch.device("cuda")
    module = ApproxNetV2LinearAttention(
        hidden_size=64,
        head_dim=8,
        num_heads=4,
        num_kv_heads=4,
...
if pytest is not None:
    @pytest.mark.skipif(_IMPORT_ERROR is not None, reason="approxnet-v2 import failed.")
    def test_approxnet_v2_dense_chunk_equivalence() -> None:
        _run_dense_chunk_equivalence_case()

    @pytest.mark.skipif(_IMPORT_ERROR is not None, reason="approxnet-v2 import failed.")
    def test_approxnet_v2_varlen_equivalence() -> None:
        _run_varlen_equivalence_case()

    @pytest.mark.skipif(_IMPORT_ERROR is not None, reason="approxnet-v2 import failed.")
    def test_approxnet_v2_triton_vs_torch() -> None:
        _run_triton_vs_torch_case()

    @pytest.mark.skipif(_IMPORT_ERROR is not None, reason="approxnet-v2 import failed.")
    def test_approxnet_v2_module_smoke() -> None:
        _run_module_smoke_case()

    @pytest.mark.skipif(_IMPORT_ERROR is not None, reason="approxnet-v2 import failed.")
    def test_approxnet_v2_triton_stateful_dense_vs_torch() -> None:
        _run_triton_stateful_dense_vs_torch_case()

    @pytest.mark.skipif(_IMPORT_ERROR is not None, reason="approxnet-v2 import failed.")
    def test_approxnet_v2_triton_stateful_varlen_vs_torch() -> None:
...
def main() -> None:
    ...
    _run_triton_stateful_varlen_vs_torch_case()
    _run_module_smoke_case()
    print("ok: approxnet-v2 tests passed")
